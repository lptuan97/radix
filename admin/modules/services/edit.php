...
// Lấy dữ liệu cũ của service
$body = getBody('get'); //Array ( [module] => services [action] => edit [id] => 5 )
print_r($body);

//Truy vấn dữ liệu từ service[id]
if(!empty($body['id'])){
    $serviceId = $body['id'];
    $serviceDetail = firstRaw("SELECT * FROM `services` WHERE id = $serviceId");
    print_r1($serviceDetail, "warning");
...
if (isPost()) {
    ...
    if (empty($errors)) {
        ...
        if ($updateStatus) {
            ...
            redirect('admin?module=services&action=edit&id='.$serviceId); //Load lại trang thêm nhóm người dùng
        }
    } else {
        //Có lỗi xảy ra
...
if(empty($old) && !empty($serviceDetail)){
    ...
    //$old = getFlashData('old'); //phải getFlashData để xóa session
}
print_r1($old, "danger");
    ...
    <div class="container-fluid">
        <form action="" method="post">
            ...
            <div class="form-group">
                ...
                <div class="row">
                    <div class="col-10">
                        <input type="text" id="ckfinder-input-1" class="form-control" name="icon" placeholder="Đường dẫn ảnh hoặc mã icon..." value="<?php echo old('icon', $old); ?>" />
                    </div>
                    <div class="col-2">
                        <button type="button" id="choose-image-add" class="btn btn-success btn-block choose-image">Chọn ảnh</button>
                    </div>
                </div>
            ...
            <div class="form-group">
                ...
                <textarea name="content" id="id_textarea" class="form-control"><?php echo old('content', $old) ?></textarea>
