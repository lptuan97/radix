...
if (isPost()) {
    ...
    foreach ($galleryArr as $key => $value) {
        if (empty(trim($value))) {
            $errors['gallery']['required'][$key] = 'ảnh không được trống' . $key;
        }
    }
    ...
    if (empty($errors)) {
        ...
        if ($insertStatus) {
            ...
            foreach ($galleryArr as $key => $value) {
                ...
                insert('portfolio_images', $dataPortfolioImage);
            }
        ...
        <form action="" method="post">
            ...
            <div class="form-group">
                ...
                <div class="row ckfinder-group">
                    <div class="col-10">
                        <input type="text" id="ckfinder-input-avatar" class="form-control image-render" name="thumbnail" placeholder="Đường dẫn ảnh..." value="<?php echo old('thumbnail', $old); ?>" />
                    </div>
                    <div class="col-2">
                        <button type="button" id="ckfinder-popup-avatar" class="btn btn-success btn-block choose-image">Chọn ảnh</button>
                    </div>
                </div>
            ...
            <div class="form-group">
                ...
                <div class="gallery-images">
                    ...
                    if (!empty($oldGallery)) {
                        function galleryImages($value, $error = '')
                        {
                            $strError = '';
                            if (!empty($error)) {
                                $strError = '<span class="error">' . $error . '</span>';
                            }
                            $strHTML = '<div class="gallery-item">
                                        <div class="row">
                                            <div class="col-11">
                                                <div class="row ckfinder-group">
                                                    <div class="col-10">
                                                        <input type="text" class="form-control image-render" name="gallery[]" placeholder="Đường dẫn ảnh..." value="' . $value . '">
                                                        ' . $strError . '
                                                        </div>
                                                    <div class="col-2">
                        ...
                        if (!empty($old['gallery'])) {
                            foreach ($oldGallery as $key => $value) {
                                if (!empty($errors['gallery']['required'][$key])) {
                                    galleryImages($value, $errors['gallery']['required'][$key]);
                                } else {
                                    galleryImages($value);
                                }
                            }
